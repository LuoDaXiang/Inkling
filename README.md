...
> 状态：v0.1.0 —— 粘一句英文能出声。评分层写了一半，录音还没开始。
...
**语调是这个项目的重点。** 参考实现 Enjoy 有语调的可视化（两条音高曲线叠加），
但没有语调的评分——它的 `prosodyScore` 那一列永远是空的。我们不但有分数，
还有逐词的单调检测和停顿检测：一个总分只能说「语调不太好」，
逐词明细能说「第 3、7、11 个词读平了」。
