/*
 * 冻结副本 —— 2026-08-29，M3 删掉 `public/` 之前的最后状态。
 *
 * 这个文件此前是 `public/present.js` 的一个**引用**，不是副本：复制一份进存档，
 * 它和本体一天就会分叉，而分叉之后这张「对照物」会安静地开始撒谎。
 *
 * M3 把本体删了，判断因此反转：没有本体可以分叉，而存档必须还打得开。
 * **不要修改这个文件。** 它的价值全部来自「它就是 M2 那一刻的样子」。
 */
/**
 * 「该展示什么」的决定层。
 *
 ...
 */
export function pitchPlot(input) {
  ...
  const series = [
    { name: "reference", contour: readContour(opts.reference) },
    { name: "recording", contour: readContour(opts.recording) },
  ].filter((s) => s.contour !== null);

  const empty = { segments: [], minHz: null, maxHz: null, durationMs: 0 };
...
function readContour(value) {
  if (!value || typeof value !== "object") return null;
  const hopMs = value.hopMs;
  if (typeof hopMs !== "number" || !Number.isFinite(hopMs) || hopMs <= 0) return null;
  if (!Array.isArray(value.hz)) return null;

  const hz = value.hz.map((v) => (typeof v === "number" && Number.isFinite(v) ? v : null));
  return { hz, hopMs };
}
 ...
 * 那正是查表结构做不到的一点。
 */
export function wordMarks(word) {
  const w = word && typeof word === "object" ? word : {};
  const errorType = typeof w.errorType === "string" ? w.errorType : "None";
  const accuracy = typeof w.accuracy === "number" && Number.isFinite(w.accuracy)
    ? w.accuracy
    : null;
 ...
 */
function baseOf(errorType, accuracy) {
 ...
 */
function monotoneOf(errorType, raw) {
...
function breakOf(errorType, raw) {
 ...
 */
export function phonemeRows(word) {
  const w = word && typeof word === "object" ? word : {};
  if (!Array.isArray(w.phonemes)) return [];

  return w.phonemes
    .filter((p) => p && typeof p === "object" && typeof p.phoneme === "string")
    .map((p) => {
      const accuracy = typeof p.accuracy === "number" && Number.isFinite(p.accuracy)
        ? p.accuracy
        : null;
      return {
        phoneme: p.phoneme,
