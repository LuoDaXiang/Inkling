...
beforeAll(async () => {
  ...
  server = await listen({ db, ...common });
  bare = await listen(common);
  base = `http://127.0.0.1:${portOf(server)}`;
  bareBase = `http://127.0.0.1:${portOf(bare)}`;
});
...
const post = (payload: unknown, at = base): Promise<Response> =>
  fetch(`${at}/api/materials`, {
    method: "POST",
...
const listRaw = (query = ""): Promise<Response> => fetch(`${base}/api/materials${query}`);
...
describe("#13 assessable:false 的句子仍然是一等公民", () => {
  test("仍然入库，仍然出现在详情里 [C18]", async () => {
    ...
    const detail = (await (await fetch(`${base}/api/materials/${made.materialId}`)).json()) as {
      sentences: Array<{ assessable: boolean }>;
    };
    expect(detail.sentences).toHaveLength(2);
    expect(detail.sentences[1]?.assessable).toBe(false);
...
describe("#4 详情与 id 校验", () => {
  test("取得到全部句子", async () => {
    ...
    const res = await fetch(`${base}/api/materials/${made.materialId}`);
    expect(res.status).toBe(200);
    const detail = (await res.json()) as Record<string, unknown>;
    expect(detail["id"]).toBe(made.materialId);
    expect((detail["sentences"] as unknown[]).length).toBe(2);
  });

  test("不存在的 id → 404", async () => {
    const res = await fetch(`${base}/api/materials/999999`);
  ...
  test("id 不是整数 → 400，不是 404 [C57]", async () => {
    ...
    for (const bad of ["abc", "1.5", "-3", "0x10", "%20"]) {
      const res = await fetch(`${base}/api/materials/${bad}`);
...
describe("C43 响应里不出现 null", () => {
  test("建材料、列表、详情三条都不发 null", async () => {
    const made = await create("One. Two.");
    for (const url of [`${base}/api/materials`, `${base}/api/materials/${made.materialId}`]) {
      expect(await (await fetch(url)).text(), url).not.toContain("null");
    }
  });
});
...
describe("没接数据库时", () => {
  test("三条路由都返回 503 unavailable，而不是 500", async () => {
    expect((await post({ title: "t", source: "paste", text: "Hi." }, bareBase)).status).toBe(503);
    expect((await fetch(`${bareBase}/api/materials`)).status).toBe(503);
    expect((await fetch(`${bareBase}/api/materials/1`)).status).toBe(503);
  });
});
